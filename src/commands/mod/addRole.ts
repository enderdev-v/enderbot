...
export default class AssignRoleCommand extends SubCommand {
  override async run(ctx: CommandContext<typeof options>) {
    ...
    const members = await ctx.client.cache.members?.values(guild?.id as string);
    
    if (role.managed) return ctx.write({ content: "No puedes asignar un rol gestionado por un bot" });

    console.log(members?.length);
    members?.forEach(async (member) => {
       const memberRoleList = await member.roles.list();
        setTimeout(() => {}, 2000);
        console.log(member.user.username);
        if (memberRoleList.some(r => r.id === role.id)) return;
