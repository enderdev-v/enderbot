...
const options = {
    title: createStringOption({
        description: "Escribe el titulo de tu articulo",
        required: true
    }),
    text: createStringOption({
        description: "Escribe el texto de tu articulo",
        required: true
    }),
    footer: createStringOption({
        description: "Escribe el footer de tu articulo",
        required: true
    }),
    ...
    button: createBooleanOption({
        description: "Quieres botones si o no?",
        required: true
    }),
    link: createStringOption({
        description: "escribe el link del boton",
    }),
    label: createStringOption({
        description: "Escribe el label del boton",
    }),
...
export default class ArticleCommand extends Command {
    override async run(ctx: CommandContext<typeof options>) {
        ...
        if (button)
            components = new Container().setColor(hex).addComponents(
            ...
            components = new Container().setColor(ctx.client.config.colors.enderbotColor).addComponents(
                new TextDisplay().setContent(`# ${title}`),
                new Separator().setSpacing(Spacing.Large), // Primer Separador
                new TextDisplay().setContent(text),
                new TextDisplay().setContent(`-# ${footer}`)
